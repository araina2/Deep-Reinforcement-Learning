...
class DQNAgent:
    ...
    def _build_model(self):
        ...
        model = Sequential()
        X_train3D = np.zeros((1, seq_size, state_size))
        #print('The state size is : ', X_train3D.shape)
    ...
    def replay(self, batch_size):
        ...
        X = np.zeros((batch_size, seq_size, self.state_size))
        Y = np.zeros((batch_size, self.action_size))
        for i in range(batch_size):
            state, action, reward, next_state, done = minibatch[i]
            state3D = np.zeros((state.shape[0], seq_size, state.shape[1]))
            next_state3D = np.zeros((state.shape[0], seq_size, state.shape[1]))
            #print('The state size for predict is : ', state3D.shape)
            for i in range(state.shape[0]):
                for j in range(seq_size):
                    count = 0
                    for k in range(state.shape[1]):
                        if (state.shape[1] - k - 1) <= j:
                            state3D[i][j][k] = state[i][count]
                            count = count + 1
            ...
            for i in range(next_state.shape[0]):
                for j in range(seq_size):
                    ...
                    for k in range(next_state.shape[1]):
                        if (next_state.shape[1] - k - 1) <= j:
                            next_state3D[i][j][k] = state[i][count]
                            count = count + 1
