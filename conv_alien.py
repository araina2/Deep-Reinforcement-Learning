...
def printUsage():
    print "Usage: python conv_alien.py <num_episodes_to_run>"
...
class DQNAgent:
    ...
    def _build_model(self):
        ...
        model = Sequential()
        model.add(Convolution2D(nb_filter=32, nb_row=5, nb_col=5, subsample=(5,5), border_mode='valid', input_shape=input_shape))
        model.add(Convolution2D(nb_filter=64, nb_row=2, nb_col=2, subsample=(2,2), border_mode='valid'))
        model.add(Convolution2D(nb_filter=64, nb_row=2, nb_col=2, subsample=(1,1), border_mode='valid'))
        model.add(Flatten())
        #model.add(LSTM(512))
    ...
    def replay(self, batch_size):
        ...
        X = np.zeros((batch_size, self.state.shape[0], self.state.shape[1], 1))
...
if __name__ == "__main__":
    ...
    for episode in xrange(EPISODES):
        ...
        while not done:
            ...
            if K.image_dim_ordering() == 'th':
                next_state = np.reshape(next_state, (1, 1, next_state.shape[0], next_stat[1]))
            else:
    ...
    outfileName = "conv"+str(EPISODES)+".h5"
